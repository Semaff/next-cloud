...
class FileService {
  /*
    ...
    =============
  */
  static async getAll(userId: string | number, curPath: string | undefined) {
    ...
    const isNotRootFolder = await File.findOne({ where: { id: parentFolder.parent } });
    if (isNotRootFolder) {
      files.push({
        id: isNotRootFolder.id,
        name: "..",
        type: "dir",
        path: isNotRootFolder.path,
        size: isNotRootFolder.size,
        parent: isNotRootFolder.parent,

        createdAt: isNotRootFolder.createdAt,
        updatedAt: isNotRootFolder.updatedAt,

        userId: isNotRootFolder.userId,
      } as File);
    }
  ...
  static async search(userId: number | string, query: string): Promise<File[]> {
    ...
    const filteredFiles = files.filter(file => {
      ...
      if (
        (fileName.startsWith(query.toLowerCase()) || (query.length >= 5 && fileName.includes(query)))
        && file.parent
      ) {
        return true;
      }
  ...
  */
  ...
  static async move(userId: number | string, fileId: number | string, parentId: number | string): Promise<File[]> {
    ...
    const returnedFiles = await FileService.getAll(userId, oldParentFolder.path.split("\\").slice(2).join("\\"));
